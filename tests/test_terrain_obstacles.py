...
def test_obstacles_replace_terrain():
    ...
    print(f"   ✅ All {single_terrain_count} passable cells have single terrain type")
    ...
    print(f"   ✅ All costs match terrain types (no stacking)")
    ...
    if maze.is_passable(test_x, test_y):
        ...
        print(f"   ✅ Obstacle replaced terrain (cost changed from {original_cost} to {new_cost})")
        print(f"   ✅ Cost does NOT stack (would be {original_cost + expected_spikes_cost} if stacked)")
    ...
    if "random.choices" in source and "[0]" in source:
        print("   ✅ assign_terrain() uses random.choices()[0] (selects ONE type)")
    else:
        print("   ⚠️  Could not verify assignment logic from source")
...
def test_cost_calculation():
    ...
    print("\n✅ All terrain costs calculated correctly")
    return True
